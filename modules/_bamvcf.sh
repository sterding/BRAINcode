...
#java -Xmx20g -jar $gatk_path/MarkDuplicates.jar INPUT=gatk_accepted_hits.bam OUTPUT=gatk_accepted_hits_marked.bam METRICS_FILE=gatk_metrics.txt CREATE_INDEX=true VALIDATION_STRINGENCY=LENIENT REMOVE_DUPLICATES=true

echo "# Indel realign"
#realignerTargetCreator
#java -Xmx20g -jar $gatk_path/GenomeAnalysisTK.jar -T RealignerTargetCreator -U ALLOW_N_CIGAR_READS -R $GENOME/Sequence/WholeGenomeFasta/genome.fa -I gatk_accepted_hits_marked.bam -o gatk_indel.intervals -known $GENOME/Annotation/Variation/Mills_and_1000G_gold_standard.indels.hg19.vcf 
echo "# IndelRealigner"
...
####java -Xmx20g -jar /PHShome/sj750/projects/convert_bam/AddOrReplaceReadGroups.jar INPUT=indelrealigned_accpeted_hits_100_marked.bam OUTPUT=indelrealigned_accpeted_rg_hits_100_marked.bam RGPL=illumina RGPU=1 RGLB=bar RGSM=march2013111 RGID=foo

echo "#index bam file"
#samtools index gatk_accepted_hits_marked_indelrealigned.bam

echo "#BaseRecalibrator"
#java -Xmx20g -jar $gatk_path/GenomeAnalysisTK.jar -T BaseRecalibrator -U ALLOW_N_CIGAR_READS -R $GENOME/Sequence/WholeGenomeFasta/genome.fa -I gatk_accepted_hits_marked_indelrealigned.bam -knownSites $GENOME/Annotation/Variation/dbsnp_138.hg19.vcf -o gatk_recal_data.table
echo "#Print reads"
#java -Xmx20g -jar $gatk_path/GenomeAnalysisTK.jar -T PrintReads -U ALLOW_N_CIGAR_READS -R $GENOME/Sequence/WholeGenomeFasta/genome.fa -I gatk_accepted_hits_marked_indelrealigned.bam -BQSR gatk_recal_data.table -o gatk_accepted_hits_marked_indelrealigned_BQSR.bam

echo "##HaplotypeCaller(after BQSR)"
# only extract SNPs in the exon regions
##java -Xmx20g -jar $PATH/gatk/GenomeAnalysisTK.jar -T HaplotypeCaller -R $GENOME/Sequence/WholeGenomeFasta/genome.fa --filter_reads_with_N_cigar -I gatk_accepted_hits_marked_indelrealigned_BQSR.bam -o gatk_accepted_hits_BQSR.vcf -D $GENOME/Annotation/Variation/dbsnp_138.hg19.vcf -stand_call_conf 50.0 -stand_emit_conf 10.0 -L $GENOME/Annotation/Genes/gencode.v13.annotation.gtf.exons.bed
# all SNPs
...
java -Xmx20g -jar $gatk_path/GenomeAnalysisTK.jar -T ApplyRecalibration -R $GENOME/Sequence/WholeGenomeFasta/genome.fa --input gatk_accepted_hits_BQSR.vcf -mode SNP --ts_filter_level 99.0 -recalFile gatak_recalibrate.SNP.recal -tranchesFile gatak_recalibrate.SNP.tranches -o accepted_hits_GATK_SNPs.vcf
...
#vcf2table
java -Xmx20g -jar $gatk_path/GenomeAnalysisTK.jar -R $GENOME/Sequence/WholeGenomeFasta/genome.fa -T VariantsToTable -V accepted_hits_GATK_SNPs.vcf -F CHROM -F POS -F ID -F REF -F ALT -F FILTER -F DP -o accepted_hits_GATK_SNPs.table
