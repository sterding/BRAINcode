...
# change MQ:255 to 4 (or any number other than 255); required by GATK
...
java -Xmx50g -jar $PATH/gatk/MarkDuplicates.jar INPUT=gatk_accepted_hits.bam OUTPUT=gatk_accepted_hits_marked.bam METRICS_FILE=gatk_metrics.txt CREATE_INDEX=true VALIDATION_STRINGENCY=LENIENT REMOVE_DUPLICATES=true

# Indel realign
#realignerTargetCreator
java -Xmx50g -jar $PATH/gatk/GenomeAnalysisTK.jar -T RealignerTargetCreator -U ALLOW_N_CIGAR_READS -R $GENOME/Sequence/WholeGenomeFasta/genome.fa -I gatk_accepted_hits_marked.bam -o gatk_indel.intervals -known $GENOME/Annotation/Variation/Mills_and_1000G_gold_standard.indels.hg19.vcf 
# IndelRealigner
...
## [optional] add read group and sort 
...
#BaseRecalibrator
java -Xmx50g -jar $PATH/gatk/GenomeAnalysisTK.jar -T BaseRecalibrator -U ALLOW_N_CIGAR_READS -R $GENOME/Sequence/WholeGenomeFasta/genome.fa -I gatk_accepted_hits_marked_indelrealigned.bam -knownSites $GENOME/Annotation/Variation/dbsnp_138.hg19.vcf -o gatk_recal_data.table
#Print reads
java -Xmx50g -jar $PATH/gatk/GenomeAnalysisTK.jar -T PrintReads -U ALLOW_N_CIGAR_READS -R $GENOME/Sequence/WholeGenomeFasta/genome.fa -I gatk_accepted_hits_marked_indelrealigned.bam -BQSR gatk_recal_data.table -o gatk_accepted_hits_marked_indelrealigned_BQSR.bam
...
java -Xmx50g -jar $PATH/gatk/GenomeAnalysisTK.jar -T ApplyRecalibration -R $GENOME/Sequence/WholeGenomeFasta/genome.fa --input gatk_accepted_hits_BQSR.vcf -mode SNP --ts_filter_level 99.0 -recalFile gatak_recalibrate.SNP.recal -tranchesFile gatak_recalibrate.SNP.tranches -o accepted_hits_GATK_SNPs.vcf
