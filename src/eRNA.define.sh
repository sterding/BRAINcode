...
# for i in HC_SNDA ILB_SNDA HC_FB HC_PBMC PD_SNDA HC_MCPY HC_TCPY; do bsub -n 1 -q normal -J $i bash $pipeline_path/src/eRNA.define.sh $i; done
# preqisition: 
...
[ -e ~/neurogen/rnaseq_PD/results/merged/samplelist.$SAMPLE_GROUP ] || _sample_list.R $SAMPLE_GROUP ~/neurogen/rnaseq_PD/results/merged/samplelist.$SAMPLE_GROUP

# RNAseq signal distribution in the background region
[ -e transcriptional.noise.rpm.txt ] || bedtools random -seed 3 -g $GENOME/Annotation/Genes/ChromInfo.txt -l 1 -n 1000000 | sortBed | intersectBed -a - -b ../toExclude.bed -v -sorted | intersectBed -a $inputBG -b - -sorted -u | cut -f4 > transcriptional.noise.rpm.txt
...
[ -e $i.rdbg ] || bedtools shuffle -seed 123 -excl ../toExclude.bed -noOverlapping -i eRNA.tmp5 -g $GENOME/Annotation/Genes/ChromInfo.txt | awk -vOFS="\t" '$4=$1"_"$2"_"$3;' | bigWigAverageOverBed $i stdin stdout | cut -f1,5 > $i.rdbg
...
echo "locus" `ls ~/neurogen/rnaseq_PD/run_output/*/uniq/accepted_hits.normalized.bw | sed 's/.*run_output\/\(.*\)\/uniq.*/\1/g' | rowsToCols stdin stdout` | sed 's/ /\t/g' | cat - $TMPFILE > eRNA.meanRPM.allSamples.xls
