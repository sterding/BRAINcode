...
JASPARID=$1
TF=$2
output=$3 # eRNA.JASPARmotifscan.txt
...
> $output"."$JASPARID;
	...
	nA=~/neurogen/tmp/xd010/NOT.$JASPARID.bed
	[ -e $nA ] || sortBed -i $A | intersectBed -a ~/neurogen/TF_scan/JASPAR2018_CORE_vertebrates_non-redundant_pfms_meme.in_genome.fa.bed -b - -v -sorted | mergeBed > $nA;
	
	#for j in eRNA eRNA-classI promoter
	...
	do
	  ...
	  echo -e "$TF\t$AB\t$AnB\t$nAB\t$nAnB\t$j" >> $output"."$JASPARID;	
	done
