...
df=read.table("eRNA.90samples.meanRPM.xls", header=T)
rownames(df) = df[,1]; df=df[,-1]

# filter HiTNE with mean expression >2.5 TPM in the SNDA
res=bigcor(t(df), fun='cor', size=1000, to.ffdf=T)

cl <- kmeans(df, 1000, iter.max=20)
m.kmeans<- cbind(df, cl$cluster) # combine the cluster with the matrix
...
pheatmap( m.kmeans[,-ncol(m.kmeans)], cluster_rows = F, cluster_cols = F, legend=FALSE, show_rownames=FALSE, show_colnames=FALSE)
...
features = read.table("eRNA.characterize.xls", header=T)
...
row_distance = dist(t(mat_data), method = "binary")  # between rows
# use binary distance: Sockal & Michener index (1958)
require(ade4)
row_distance = dist.binary(mat_data, method = 2)
...
heatmap.2(mat_data,
  ...
  Rowv = as.dendrogram(row_cluster))            # turn off column clustering
...
#read random
rd=read.table("intron.length.n_HITNE.random.txt", header=F, stringsAsFactors =F); colnames(rd)=c('intronID', 'len_intron','n_HITNE')
fd=read.table("intron.length.n_HITNE.txt", header=F, stringsAsFactors =F);  colnames(fd)=c('intronID', 'len_intron','n_HITNE')
...
rd=read.table("Hostgene.length.nHITNErandom.metaExon.metaIntron.xls", header=F, stringsAsFactors =F); colnames(rd)=c('gene_symbol','gene_ENSID','gene_type', 'len_gene','n_HITNE','len_exon','len_intron')
...
plot(fd$len_intron/1000, fd$n_HITNE, main="", xlim=range(rd$len_intron/1000, fd$len_intron/1000), col='orangered', cex.axis=0.8, cex=.6, pch=1, ylab="Number of HiTNEs in host gene", xlab="Total length of host gene introns (Kb)")
points(rd$len_intron/1000, rd$n_HITNE, cex=.6, pch=3, col='darkblue')
legend("topleft", c("genes w/ intronic HiTNEs","genes w/ randomly scrambled HiTNEs"), col=c('orangered','darkblue'), pch=c(1,3), cex=.8, bty='n')
