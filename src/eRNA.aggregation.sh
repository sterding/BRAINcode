...
#============================================================
# Download bigwig files for various marks
#============================================================
bash ~/neurogen/pipeline/RNAseq/src/get.regulatoryMarks.data.sh
...
intersectBed -a eRNA.bed -b ../DNase/E081-DNase.macs2.narrowPeak -wao | sort -k4,4 -k15,15nr | awk '{if($4!=id) {id=$4;print;}}' | awk '{OFS="\t"; mid=($5==".")?int(($2+$3)/2):($6+$14); print $1, mid-1000, mid+1000,$4;}'> eRNA.1kbp.summit.or.mid.1kbp.bed

# if overlapping with any DNase peak (from any brain), take the summit of the peak; otherwise, take the center point of HiTNEs. Then expand [-1k,+1k] from whatever the point determined above.
#intersectBed -a eRNA.bed -b ../DNase/E081-DNase.macs2.narrowPeak -wao | sort -k4,4 -k15,15nr | awk '{if($4!=id) {id=$4;print;}}' | awk '{OFS="\t"; mid=($5==".")?int(($2+$3)/2):($6+$14); print $1, mid-1000, mid+1000,$4;}'> eRNA.1kbp.summit.or.mid.1kbp.bed

cat eRNA.bed | while read chr start end name score strand rest
do
    N=`expr $end - $start`;
    s=`bigWigSummary ../DNase/E081-DNase.pval.signal.bigwig -udcDir=/tmp -type=mean $chr $start $end $N 2>/dev/null | sed 's/n\/a/0/g'`
    [[ $s == "" ]] && whichmax=`expr $N / 2`
...
for i in ../*/*.bigwig;
do
    [ -e $i.eRNA.1kbp.mid.1kbp.100bins ] || toBinRegionsOnBigwig.sh $i eRNA.1kbp.mid.1kbp.bed 100 > $i.eRNA.1kbp.mid.1kbp.100bins &
