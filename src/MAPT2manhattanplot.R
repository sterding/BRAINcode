#############################################################################################
#
# Rscript to make manhatten plot for eQTL SNP rs17649553 in MAPT locus (chr17:43000000-45300000), for both BRAINCODE genes and HTNEs
#
# output: ~/Dropbox/PDBrainMap/manuscript/MAPT/eQTLplot.rs17649553.pdf
#############################################################################################
LEFT=43000000; RIGHT=45300000; querySNP='rs17649553'
### Gene eQTL from BrainCODE
## ===================
df=read.table("~/neurogen/rnaseq_PD/results/eQTL/HCILB_SNDA/final.cis.eQTL.d1e6.p1e-2.xls", header=T, stringsAsFactors = F)
...
df=subset(df, SNP.chr=='chr17' & grepl(querySNP,SNP) & start<=RIGHT & end>=LEFT)
df0=df
require("ggplot2")
p <- ggplot(df0, aes(x=end, y=-log10(p.value))) 
p = p + geom_point(aes(fill = geneSymbol), colour='black', pch=21, size = 1)


### HTNE eQTL from BrainCODE
## ===================
df=read.table("~/eRNAseq/HCILB_SNDA/final.cis.eQTL.d1e6.p1e-2.xls", header=T, stringsAsFactors = F)
head(df)
df=cbind(df, start=do.call(rbind,strsplit(as.character(df$gene),"_"))[,3], end=do.call(rbind,strsplit(as.character(df$gene),"_"))[,3])
df$start = as.numeric(as.character(df$start)); df$end = as.numeric(as.character(df$end))
df=subset(df, SNP.chr=='chr17' & grepl(querySNP,SNP) & start<=RIGHT & end>=LEFT)

p + geom_point(aes(x=end, y=-log10(p.value), fill = gene),data=df, colour='magenta', pch=21, size = 1) + ylim(0, 8) + scale_x_continuous(limits=c(LEFT, RIGHT), expand = c(0,0))

ggsave(height=1.5,width=14,dpi=200, filename='~/Dropbox/PDBrainMap/manuscript/MAPT/eQTLplot.rs17649553.pdf', useDingbats=FALSE)

#############################################################################################
#
# Rscript to make manhatten like plot for eQTL in MAPT locus (chr17:43583680-44506586), using GTEx and BRAINCODE result
#
# output: ~/Dropbox/PDBrainMap/manuscript/MAPT/eQTLplot.MAPT.pdf
#############################################################################################
