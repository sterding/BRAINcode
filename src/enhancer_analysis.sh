...
# regions to exclude (e.g. 500nt flanking of TSS, 500nt flanking of exons)
...
# regions to exclude (e.g. 500nt flanking of any exons)
bedtools bed12tobed6 -i $ANNOTATION/gencode.v19.annotation.bed12 | slopBed -g $GENOME/Sequence/WholeGenomeFasta/genome.fa.fai -b 500 > toExclude.bed
...
#for i in /data/neurogen/rnaseq_PD/results/merged/*trimmedmean.uniq.normalized.bedGraph; do echo $i; awk '$4>0.05' $i | mergeBed | intersectBed -a - -b ../toExclude.bed -v | awk '($3-$2)>=50' > ${i/bedGraph/eRNA.bed} & done
awk '$4>0.05' /data/neurogen/rnaseq_PD/results/merged/HC_SNDA.trimmedmean.uniq.normalized.bedGraph | mergeBed -d 100 | intersectBed -a - -b ../toExclude.bed -v | awk '($3-$2)>=200' > /data/neurogen/rnaseq_PD/results/merged/HC_SNDA.trimmedmean.uniq.normalized.eRNA.bed
awk '{OFS="\t"; mid=int(($3+$2)/2); if(mid>500) print $1, mid-500, mid+500;}' /data/neurogen/rnaseq_PD/results/merged/HC_SNDA.trimmedmean.uniq.normalized.eRNA.bed > RNAseq.distal.bed

ln -s /data/neurogen/rnaseq_PD/results/merged/HC_SNDA.trimmedmean.uniq.normalized.bw RNAseq.distal.bigwig

# strigent version of eRNA
for i in /data/neurogen/rnaseq_PD/results/merged/*trimmedmean.uniq.normalized.bedGraph; do echo $i; awk '$4>=0.5' $i | mergeBed -d 100 | intersectBed -a - -b ../toExclude.bed -v > ${i/bedGraph/eRNA.strong.bed} & done
